...
class ReviewViewController: UICollectionViewController, UICollectionViewDelegateFlowLayout {
    ...
    class func instantiateFromStoryboard() -> ReviewViewController {
        ...
        return storyboard.instantiateViewController(withIdentifier: String(describing: self)) as! ReviewViewController
    }
    ...
    func fetchPosts(){
        if !lastPageFetched {
            _ = BlossomRequest.request(method: .get, endPoint: "\(Api.posts)?page=\(postPage)&post_type=review") { (response, statusCode, json) -> () in
                if statusCode == 200{
