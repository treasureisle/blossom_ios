...
//
//  https://github.com/Rannie/Toast-Swift 편집
//
...
// image size
let HRToastImageViewWidth :  CGFloat  = 50.0
let HRToastImageViewHeight:  CGFloat  = 50.0

// label setting
let HRToastMaxWidth       :  CGFloat  = 0.8;      // 80% of parent view width
let HRToastMaxHeight      :  CGFloat  = 1.0;
let HRToastFontSize       :  CGFloat  = 15.0
...
extension UIView {
    ...
    func shake(_ duration:TimeInterval = 0.07, power:CGFloat = 10){
        let animation = CABasicAnimation(keyPath: "position")
        animation.duration = duration
        animation.repeatCount = 3
        animation.autoreverses = true
        animation.fromValue = NSValue(cgPoint: CGPoint(x: self.center.x - power, y: self.center.y))
        animation.toValue = NSValue(cgPoint: CGPoint(x: self.center.x + power, y: self.center.y))
    ...
    func makeErrorToast(message msg: String){
        let errorImage = UIImage(named: ImageName.unhappy)
        self.makeToast(message:msg, duration:2, position:HRToastPositionCenter as AnyObject, title:"", image: errorImage!)
    }
    
    func makeSomethingWrongToast(){
        makeErrorToast(message: NSLocalizedString("SOMETHING_WENT_WRONG", comment: "서버 에러가 났을때 표시"))
    }
    
    func makeCenterToast(message msg: String){
    ...
    func centerPointForPosition(_ position: AnyObject, toast: UIView) -> CGPoint {
        ...
        log.debug("Warning: Invalid position for toast.")
    ...
    func viewForMessage(_ msg: String?, title: String?, image: UIImage?) -> UIView? {
        ...
        if imageView != nil {
            ...
            imageLeft = HRToastHorizontalMargin + (imageWidth / 2)
            imageView!.frame.origin.x = imageLeft
        } else {
        ...
        if titleLabel != nil {
            ...
            titleTop = HRToastVerticalMargin * 2 + imageHeight
            titleLeft = HRToastHorizontalMargin
        } else {
        ...
        if msgLabel != nil {
            ...
            msgLeft = HRToastHorizontalMargin
        } else {
        ...
        let wrapperHeight = msgTop + msgHeight + HRToastVerticalMargin
        ...
        if imageView != nil {
            ...
            imageView!.frame.origin.y = HRToastVerticalMargin
        }
